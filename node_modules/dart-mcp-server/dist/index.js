...
import { ListToolsRequestSchema, CallToolRequestSchema, ListResourceTemplatesRequestSchema, ReadResourceRequestSchema, ListPromptsRequestSchema, GetPromptRequestSchema, } from "@modelcontextprotocol/sdk/types.js";
import { readFileSync } from "fs";
import { join, dirname } from "path";
import { fileURLToPath } from "url";
import { ApiError, ConfigService, DocService, TaskService, } from "dart-tools";
...
const packageJson = JSON.parse(readFileSync(join(dirname(filename), "..", "package.json"), "utf-8"));
const getIdValidated = (strMaybe) => {
...
// Resources
const CONFIG_RESOURCE_TEMPLATE = {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        switch (request.params.name) {
            ...
            case "create_task": {
                const taskData = request.params.arguments;
            ...
            case "create_doc": {
                const docData = request.params.arguments;
        ...
        if (error instanceof ApiError) {
            throw new Error(`API error: ${error.status} ${JSON.stringify(error.body) || error.message || "(unknown error)"}`);
        }
        throw error;
    }
});
